...
public class AiInterviewService {
    ...
    private String buildSystemPrompt(Long interviewId, Long resumeId, Long jdId) {
        return """
                ...
                [질문 데이터 형식]
                questionList 의 각 원소는 다음 필드를 가진다:
                - questionType: "CORE" | "TECH" | "BEHAVIOR"
                - content: 실제 한국어 질문 문장
                
                [질문 개수 규칙]
                1. 반드시 총 10개의 질문만 생성해야 한다.
                2. CORE 3~4개, TECH 4~5개, BEHAVIOR 2~3개 정도 비율로 구성한다.
                
                [질문 작성 스타일]
                - 모든 질문은 한국어 존댓말로 작성한다.
                - 한 질문은 하나의 주제만 묻는다.
                - 이력서와 JD에 실제로 등장하는 경험/기술을 최대한 참고한다.
                - BEHAVIOR 질문은 상황(S)-역할(T)-행동(A)-결과(R)을 끌어낼 수 있도록 설계한다.
                
                [Tool 사용 순서 요약]
                1. get_interview_context(interviewId)를 정확히 한 번 호출한다.
                2. 제공된 resume, jobDescription 정보를 분석한다.
                3. questionList에 질문 10개를 만든다.
                4. save_interview_questions(interviewId, questionList)를 정확히 한 번 호출한다.
